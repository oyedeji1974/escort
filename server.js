...
mongoose.connect(dbURI)
    .then(() => {
        ...
        // 6. Start the Server only after successful DB connection
        app.listen(PORT, () => {
    ...
    .catch((err) => {
        ...
        console.log('\nQuick Fix Checklist:');
        console.log('1. Is your IP whitelisted in MongoDB Atlas Network Access?');
        console.log('2. Is your password in .env correct?');
        console.log('3. Is your internet connection active?');
        console.log('--------------------------------------------');
    });

// 7. Sample Routes
app.get('/', (req, res) => {
    res.status(200).json({ message: "Welcome to the Escort Server API" });
});
